...
RUN microdnf update -y && \
    ...
    microdnf install -y --allowerasing glibc-langpack-en python311 openssl nginx nginx-mod-stream java-11-openjdk-headless java-17-openjdk-headless java-21-openjdk-headless tzdata-java fontconfig binutils curl && \
...
ENV PYTHON_BUILD_RPMS="python3.11-pip python3.11-devel libffi-devel gcc"
RUN microdnf install -y $PYTHON_BUILD_RPMS  && \
    python -m pip install requests pyyaml jinja2 backoff httplib2 distro cryptography psycopg2-binary  && \
